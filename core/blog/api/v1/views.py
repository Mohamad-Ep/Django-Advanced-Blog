...
from rest_framework.generics import (GenericAPIView,ListAPIView,ListCreateAPIView,RetrieveAPIView,RetrieveDestroyAPIView,
...
def post_details(request,id):
    if request.method == 'GET':
        ...
        post = get_object_or_404(Post,id=id)
        post.delete()
        return Response({'details':'Object Delete is Successfully'},status=status.HTTP_204_NO_CONTENT)
# __________________________________________________________
...
class PostViewSet(viewsets.ModelViewSet):
    ...
    filterset_fields = {'category':['exact','in'], 'author':['exact']}          # در حالت اگزکت میشه خودش و درحالت این میشه چندتا کتگوری رو باهم فیلتر کرد
